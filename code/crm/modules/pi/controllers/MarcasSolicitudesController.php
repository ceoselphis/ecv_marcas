...
class MarcasSolicitudesController extends AdminController
{
  ...
  private function LoadPage()
  {
    ...
    return $CI->load->view('marcas/solicitudes/create', [
      ...
      'oficinas'              => $CI->MarcasSolicitudes_model->findAllOficinas(),
      'clientes'              => $CI->MarcasSolicitudes_model->findAllClients(),
      'solicitantes'          => $CI->MarcasSolicitudes_model->findAllPropietarios2(),
  ...
  public function update(string $id = null)
  {
    ...
    if ($file != NULL) {
      //We fill the data of the         
      $fpath = FCPATH . 'uploads/marcas/signos/' . md5($form['id'] . '-' . $file['name']);
      $path = site_url('uploads/marcas/signos/' . md5($form['id'] . '-' . $file['name']));
      move_uploaded_file($file['tmp_name'], $fpath);
      $solicitud['signo_archivo'] = $path;
    }
    ...
    unset($isset);
    $isset = !empty(json_decode($form["clase_niza"], TRUE)) ? $CI->MarcasSolicitudes_model->deleteClasesNizaBySolicitud($id) : false;
    if ($isset) {
    ...
    unset($isset);*/
    $isset = !empty(json_decode($form["solicitantes_id"])) ? $CI->MarcasSolicitudes_model->deleteMarcasSolicitantesBySolicitud($id) : false;
    if ($isset) {
      /*Seteamos el arreglo para los solicitantes */
      foreach (json_decode($form['solicitantes_id'], TRUE) as $row) {
        $solicitantes[] = [
