...
class MarcasSolicitudesDocumentoController extends AdminController
{
  ...
  public function addSolicitudDocumento()
  {
    ...
    $file = '';
    $doc_arch = '';
    if (!empty($_FILES['doc_archivo']) || $data['doc_archivo'] != 'undefined') {
      $file = $_FILES['doc_archivo'];
    }
    if ($file != NULL) {
      //We fill the data of the    
      $info = new SplFileInfo($file['name']);
      $filename = md5($file['tmp_name']) . "." . $info->getExtension();
      $fpath = FCPATH . 'uploads/marcas/documentos/' . $filename;
      $path = site_url('uploads/marcas/documentos/' . $filename);
      if (move_uploaded_file($file['tmp_name'], $fpath)) {
        $doc_arch = $path;
      }
      else {
        echo json_encode(['message' => 'Archivo Invalido']);
        die;
      }
    }
    /*$doc_arch = '';
  ...
  public function edit(string $id = null)
  {
    ...
    if (isset($query)) {
      ...
      return $CI->load->view('solicitud_documento/edit', [
        'labels' => $labels, 'values' => $query, 'id' => $id,
