...
class PropietariosController extends AdminController
{
            ...
            if (!empty($query)) {
                foreach ($query as $row) {
                    ...
                    $result[] = array(
                      ...
                      "acciones" => "<a href='". admin_url("pi/PropietarosController") ."/edit/{$row['id']}' class='btn btn-sm btn-primary'> editar</a>"
                    );
                }
        ...
        if(!empty($propietarios))
        {
          foreach($propietarios as $key => $value)
          {
            $data[] = [
              ...
              "pais" => $value["pais"],
              "actividad_comercial" => $value["actividad_comercial"],
              "acciones" => "<a href='". admin_url("pi/PropietarosController") ."/edit/{$value['id']}' class='btn btn-sm btn-primary'> editar</a>"
            ];
          }
        }
        else
    ...
    public function show(string $id = null)
    {
        $CI = &get_instance();
        $CI->load->model("Propietarios_model");
        $propietarioQuery = $CI->Propietarios_model->find($id);
        $dataProp = array();
        $result = array();
        $paises = $CI->Propietarios_model->getAllPaises();
        //We check if propietario exists
        if (!empty($propietarioQuery)) {
            foreach($propietarioQuery as $key => $value)
            {
                $dataProp = $value;
            }
            $query = $CI->Propietarios_model->searchMarcaByPropietario($id);
            if (!empty($query)) {
                foreach ($query as $row) {
                    $result[] = [
                        'id' => $row['id'],
            ...
            return $CI->load->view('propietarios/show', ['paises' => $paises, 'propietario' => $dataProp, 'marcas' => $result]);
        }
        else {
            return redirect('/404');
        }
