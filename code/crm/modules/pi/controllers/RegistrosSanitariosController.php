...
class RegistrosSanitariosController extends AdminController
{
    protected $models = ['RegistrosSanitarios_model'];
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'Responsables'           => $CI->RegistrosSanitarios_model->findAllStaff(),
            'tipo_solicitud'        => $CI->RegistrosSanitarios_model->findAllTipoSolicitud(),
            'estados_solicitudes'   => $CI->RegistrosSanitarios_model->findAllEstadosSolicitudes(),
            'id_pais'               => $CI->RegistrosSanitarios_model->findAllPaises(),
    ...
    private function getFields()
    {
        $CI = &get_instance();
        $CI->load->model("RegistrosSanitarios_model");
        $fields = $CI->RegistrosSanitarios_model->getFillableFields();
        $inputs = array();
        //$labels = array();
    ...
    private function LoadPage(){
        $CI = &get_instance();
        $CI->load->model("RegistrosSanitarios_model");
        $id = intval($CI->RegistrosSanitarios_model->setCountPK());

        $inputs = $this->getFields();

        $data = array();
        $tarea = $CI->RegistrosSanitarios_model->findAllTareasTipos();
        foreach ($tarea as $row) {
            $data[] = array(
                'id' => $row['id'],
                'tipo_tarea' => $row['nombre'],//$CI->RegistrosSanitarios_model->BuscarTipoTareas($row['id_tipo_tareas']),
                'descripcion' => $row['descripcion'],
                'fecha' => $row['fecha'],
            );
        }
        $eventos = $CI->RegistrosSanitarios_model->findAllEventosTipos();
        $datos = array();
        foreach ($eventos as $row) {
            $datos[] = array(
                'id' => $row['id'],
                'tipo_evento' => $row['descripcion'],//$CI->RegistrosSanitarios_model->BuscarTipoEventos($row['id_tipo_evento']),
        ...
        return $CI->load->view('registros_sanitarios/create', [
            ...
            'responsable'           => $CI->RegistrosSanitarios_model->findAllStaff(),
            'tipo_solicitud'        => $CI->RegistrosSanitarios_model->findAllTipoSolicitud(),
            'estados_solicitudes'   => $CI->RegistrosSanitarios_model->findAllEstadosSolicitudes(),
            'id_pais'               => $CI->RegistrosSanitarios_model->findAllPaises(),
            'tipo_evento'           => $CI->RegistrosSanitarios_model->findAllTipoEvento(),
            'SolDoc'                => $CI->RegistrosSanitarios_model->findAllSolicitudesDocumento(),
            'tipo_tareas'           => $CI->RegistrosSanitarios_model->findAllTipoTareas(),
   ...
   /*
     public function store()
    {
        ...
        else
        {
            ...
            try {
                $CI->RegistrosSanitarios_model->insert($solicitud);
                $CI->RegistrosSanitarios_model->insertAutoresDesignados($autoresSol);
                $CI->RegistrosSanitarios_model->insertSolicitantesDesignados($propietariosSol);
   ...
   */
    ...
    private function LoadPageEdit(string $id = null){
        ...
        $cod_contador = 'I-' . $CI->RegistrosSanitarios_model->CantidadSolicitudes();
    ...
    //         unset($isset);
    //         $isset = $CI->RegistrosSanitarios_model->deleteAutoresDesignadosBySolicitud($id);
    //         if ($isset) {
    ...
    //         } 
    //         try {
    //             $CI->RegistrosSanitarios_model->update($id, $solicitud);
    //             $CI->RegistrosSanitarios_model->insertAutoresDesignados($autoresSol);
    //             $CI->RegistrosSanitarios_model->insertSolicitantesDesignados($propietariosSol);
    ...
    public function destroy(string $id)
    {
        ...
        $CI->load->model("RegistrosSanitarios_model");
        $CI->load->helper('url');
        $query = $CI->RegistrosSanitarios_model->delete($id);
            ...
            if ($row != '[]') {
                switch ($row) {
                    case 'pais_id':
                        ...
                        $result[] = $CI->RegistrosSanitarios_model->searchWhere($q, 2);
                        break;
                    case 'boletin_id':
                        ...
                        $result[] = $CI->RegistrosSanitarios_model->searchWhere($q, 1);
                        break;
                    case 'client_id':
                        ...
                        $result[] = $CI->RegistrosSanitarios_model->searchWhere($q, 4);
                        break;
                    case 'oficina_id':
                        ...
                        $result[] = $CI->RegistrosSanitarios_model->searchWhere($q, 8);
                        break;
                    case 'staff_id':
                    ...
                    case 'tip_sol_id':
                        ...
                        $result[] = $CI->RegistrosSanitarios_model->searchWhere($q,);
                        break;
                    case 'est_sol_id':
                        ...
                        $result[] = $CI->RegistrosSanitarios_model->searchWhere($q, 9);
                        break;
                    case 'tip_signo_id':
                        ...
                        $result[] = $CI->RegistrosSanitarios_model->searchWhere($q, 6);
                        break;
                }
            }
        }
    ...
    public function insertClases()
    {
        ...
        $query = $CI->RegistrosSanitarios_model->insertMarcasClases($params);
        if ($query) {
    ...
    public function getClasesMarcas($marcas_id = NULL)
    {
        ...
        $CI->load->helper(['url', 'form']);
        $CI->load->library('form_validation');
        $query = $CI->RegistrosSanitarios_model->getMarcasClases($marcas_id);
        $result = array();
        if (!empty($query)) {
            foreach ($query as $row) {
    ...
    public function getInvoicesByMarca($marcaid = null)
    {
        $CI = &get_instance();
        $CI->load->model('RegistrosSanitarios_model');
        $query = $CI->RegistrosSanitarios_model->getInvoicesByMarca($marcaid);
        $response = array();
        if (!empty($query)) {
            foreach ($query as $key => $value) {
                $invoice = $CI->RegistrosSanitarios_model->getInvoice($value['facturas_id']);
                $row = array();
