...
class CambioDomicilioController extends AdminController
{
    protected $models = ['PatentesCambioDomicilio_model'];
    ...
    public function create()
    {
        ...
        */
        ...
        if (!empty($data)){
            ...
            $query = $CI->PatentesCambioDomicilio_model->insert($insert);
            if (isset($query)){
                echo json_encode(['message' => 'Cambio Nombre Guardado Correctamente' , 'code' => '200']);
            }else {
                echo json_encode(['message' => 'Error al Guardar la Cambio Nombre' , 'code' => '500']);
            }
        }else {
  ...
  public function turn_dates($date)
  {
    ...
    public function edit(string $id = null)
    {
        ...
        $CI->load->model("PatentesCambioDomicilio_model");
        $CI->load->helper('url');
        $query = $CI->PatentesCambioDomicilio_model->find($id);
        if(isset($query))
        {
          $patente[] = [
              "id" => $query[0]['id'] ,
            ...
            $data = [
                'id'            => $id,
                'tipo_registro' => $CI->PatentesCambioDomicilio_model->getTipoSolicitudes(),
                'clientes'      => $CI->PatentesCambioDomicilio_model->getAllClients(),
                'oficinas'      => $CI->PatentesCambioDomicilio_model->getAllOficinas(),
                'estado' => $CI->PatentesCambioDomicilio_model->getAllEstadoExpediente(),
                'responsable'   => $CI->PatentesCambioDomicilio_model->getAllStaff(),
                'pais_id'       => $CI->PatentesCambioDomicilio_model->getAllPaises(),
                'inventores'    => $CI->PatentesCambioDomicilio_model->getAllInventores(),
                'cod_contador'  =>  "P-{$id}",
                'solicitantes'  => $CI->PatentesCambioDomicilio_model->getAllClients(),
                'solicitantes_selected' => $CI->PatentesCambioDomicilio_model->findPatenteSolicitantes($id),
                'inventores_selected' => $CI->PatentesCambioDomicilio_model->findPatenteInventores($id),
                'projects' => $CI->PatentesCambioDomicilio_model->findAllProjects(),
                'tareas' => $CI->PatentesCambioDomicilio_model->findAllTipoTarea(),
                'values' => $patente,
                'tipo_evento' => $CI->PatentesCambioDomicilio_model->findAllTipoEvento(),
    ...
    public function update(string $id = null)
    {
        ...
        if ($CI->form_validation->run() == FALSE) {
            ...
            //We prepare the data 
            $query = $CI->PatentesCambioDomicilio_model->update($id, $data);
            if (isset($query))
            {
                return redirect('pi/patentes/SolicitudesController');
            }
