...
{
    protected $models = ['PatentesCesiones_model'];
    ...
    public function create()
    {
        ...
        /* 
            ...
            `client_id` int(11) DEFAULT NULL COMMENT 'FK con la tabla clientes',
            `oficina_id` int(11) NOT NULL,
            `patentes_id` int(11) NOT NULL,
            `staff_id` int(11) DEFAULT NULL COMMENT 'FK con la tabla staff',
            `estado_id` int(11) NOT NULL,
            `solicitud_num` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_spanish_ci NOT NULL,
            `fecha_solicitud` date NOT NULL,
            `resolucion_num` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_spanish_ci NOT NULL,
            `fecha_resolucion` date NOT NULL,
            `referencia_cliente` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_spanish_ci NOT NULL,
        ...
        */
        ...
        if (!empty($data)){
            ...
            $query = $CI->PatentesCesiones_model->insert($insert);
            if (isset($query)){
                echo json_encode(['message' => 'Cesion Guardado Correctamente' , 'code' => '200']);
            }else {
                echo json_encode(['message' => 'Error al Guardar la Cesion' , 'code' => '500']);
            }
        }else {
  ...
  public function turn_dates($date)
  {
    ...
    public function edit(string $id = null)
    {
        ...
        $CI->load->model("PatentesCesiones_model");
        $CI->load->helper('url');
        $query = $CI->PatentesCesiones_model->find($id);
        if(isset($query))
        {
          $patente[] = [
              "id" => $query[0]['id'] ,
            ...
            $data = [
                'id'            => $id,
                'tipo_registro' => $CI->PatentesCesiones_model->getTipoSolicitudes(),
                'clientes'      => $CI->PatentesCesiones_model->getAllClients(),
                'oficinas'      => $CI->PatentesCesiones_model->getAllOficinas(),
                'estado' => $CI->PatentesCesiones_model->getAllEstadoExpediente(),
                'responsable'   => $CI->PatentesCesiones_model->getAllStaff(),
                'pais_id'       => $CI->PatentesCesiones_model->getAllPaises(),
                'inventores'    => $CI->PatentesCesiones_model->getAllInventores(),
                'cod_contador'  =>  "P-{$id}",
                'solicitantes'  => $CI->PatentesCesiones_model->getAllClients(),
                'solicitantes_selected' => $CI->PatentesCesiones_model->findPatenteSolicitantes($id),
                'inventores_selected' => $CI->PatentesCesiones_model->findPatenteInventores($id),
                'projects' => $CI->PatentesCesiones_model->findAllProjects(),
                'tareas' => $CI->PatentesCesiones_model->findAllTipoTarea(),
                'values' => $patente,
                'tipo_evento' => $CI->PatentesCesiones_model->findAllTipoEvento(),
    ...
    public function update(string $id = null)
    {
        ...
        if ($CI->form_validation->run() == FALSE) {
            ...
            //We prepare the data 
            $query = $CI->PatentesCesiones_model->update($id, $data);
            if (isset($query))
            {
                return redirect('pi/patentes/SolicitudesController');
            }
