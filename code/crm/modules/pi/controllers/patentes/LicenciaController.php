...
{
    protected $models = ['PatentesLicencia_model'];
    ...
    public function create()
    {
        ...
        /* 
            ...
            `client_id` int(11) DEFAULT NULL COMMENT 'FK con la tabla client',
        ...
        */
        ...
        if (!empty($data)){
            ...
            $query = $CI->PatentesLicencia_model->insert($insert);
            if (isset($query)){
                echo json_encode(['message' => 'Licencia Guardado Correctamente' , 'code' => '200']);
            }else {
  ...
  public function turn_dates($date)
  {
    ...
    public function edit(string $id = null)
    {
        ...
        $CI->load->model("PatentesLicencia_model");
        $CI->load->helper('url');
        $query = $CI->PatentesLicencia_model->find($id);
        if(isset($query))
        {
          $patente[] = [
              "id" => $query[0]['id'] ,
            ...
            $data = [
                'id'            => $id,
                'tipo_registro' => $CI->PatentesLicencia_model->getTipoSolicitudes(),
                'clientes'      => $CI->PatentesLicencia_model->getAllClients(),
                'oficinas'      => $CI->PatentesLicencia_model->getAllOficinas(),
                'estado' => $CI->PatentesLicencia_model->getAllEstadoExpediente(),
                'responsable'   => $CI->PatentesLicencia_model->getAllStaff(),
                'pais_id'       => $CI->PatentesLicencia_model->getAllPaises(),
                'inventores'    => $CI->PatentesLicencia_model->getAllInventores(),
                'cod_contador'  =>  "P-{$id}",
                'solicitantes'  => $CI->PatentesLicencia_model->getAllClients(),
                'solicitantes_selected' => $CI->PatentesLicencia_model->findPatenteSolicitantes($id),
                'inventores_selected' => $CI->PatentesLicencia_model->findPatenteInventores($id),
                'projects' => $CI->PatentesLicencia_model->findAllProjects(),
                'tareas' => $CI->PatentesLicencia_model->findAllTipoTarea(),
                'values' => $patente,
                'tipo_evento' => $CI->PatentesLicencia_model->findAllTipoEvento(),
    ...
    public function update(string $id = null)
    {
        ...
        if ($CI->form_validation->run() == FALSE) {
            ...
            //We prepare the data 
            $query = $CI->PatentesLicencia_model->update($id, $data);
            if (isset($query))
            {
                return redirect('pi/patentes/SolicitudesController');
            }
