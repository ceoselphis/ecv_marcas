...
class MarcasSolicitudes_model extends BaseModel
{
    ...
    public function findPaisesDesignados($id = NULL)
    {
        ...
        $this->db->from('tbl_marcas_solicitudes_paises');
    ...
    public function searchWhere2($params): array{
        $this->db->select('id, cod_contador, nombre_tipo_registro as tipo_registro, nombre_propietario, marca, nombre_niza as clase_niza, nombre_solictud as estado_expediente, nombre_pais_solicitud as pais_nom, num_solicitud as solicitud, fecha_solicitud, num_registro as registro, certificado, fecha_vencimiento, fecha_registro');
        $this->db->distinct();
    ...
    {
        $this->db->select('a.id, a.cod_contador, b.nombre as tipo_registro, d.nombre_propietario, a.signonom as marca ,f.nombre as clase_niza, g.nombre as estado_expediente, i.nombre as pais_nom, a.solicitud, a.fecha_solicitud, a.registro,  a.fecha_registro, a.certificado, a.fecha_vencimiento');
        $this->db->distinct();
        $this->db->from('tbl_marcas_solicitudes a');
        $this->db->join('tbl_marcas_tipo_registro b', 'a.tipo_registro_id = b.id', 'left outer');
        $this->db->join('tbl_marcas_solicitantes c', 'a.id = c.marcas_id', 'left outer');
