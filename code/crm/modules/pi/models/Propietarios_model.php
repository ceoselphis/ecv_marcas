...
class Propietarios_model extends BaseModel
{
    ...
    {
      $this->db->from("tblview_propietarios");
    ...
    public function searchWhere($params): array{
        $this->db->select("a.id, a.codigo, (CASE
    ...
    END) AS `estado_civil`, a.representante_legal, c.nombre as pais, a.actividad_comercial, a.nombre_propietario, c.nombre, b.numero, a.created_at, d.firstname as firstname_created, d.lastname as lastname_created, a.modified_at, e.firstname as firstname_modif, e.lastname as lastname_modif");
    ...
    public function searchMarcaByPropietario($propietario_id): array 
    {
        $this->db->select("c.id, c.cod_contador, e.nombre as pais_nombre, c.signonom, c.signo_archivo, f.userid, f.code, f.company, g.nombre, c.id as marca_id");
        $this->db->from("tbl_propietarios a");
        $this->db->join("tbl_marcas_solicitantes b", "a.id = b.propietario_id");
        $this->db->join("tbl_marcas_solicitudes c", "c.id = b.marcas_id");
        $this->db->join("tbl_marcas_solicitudes_paises d", "c.id = d.marcas_id");
        $this->db->join("tbl_paises e", "d.pais_id = e.id");
        $this->db->join("tblclients f", "c.client_id = f.userid");
        $this->db->join("tbl_estado_expediente g", "g.id = c.estado_id");
        $this->db->where("a.id = {$propietario_id}");
        $query = $this->db->get();
        return $query->result_array();
    }
