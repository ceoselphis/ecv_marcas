...
class TipoMarcasDomicilio_model extends BaseModel
{
    protected $primaryKey = 'domicilio_id';
    protected $tableName =  'tbl_marcas_domicilios';
    ...
    public function BuscarCambioDomicilio($id = NULL){
        $this->db->select('*');
    ...
    public function findAllCambioDomicilio($id = NULL){
        $this->db->select('*');
        $this->db->from('tbl_marcas_domicilios');
        $this->db->where('cambio_domicilio_id = '.$id);
        $query = $this->db->get();
        $values = $query->result_array();
        return $values; 
    }
