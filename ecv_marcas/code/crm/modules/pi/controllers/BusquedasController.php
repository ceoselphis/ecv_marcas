  ...
  public function getBusquedas()
  {
    ...
    if (!empty($query)) {
      foreach ($query as $key => $value) {
        //$busquedaInt = ('' || is_null($value['busqueda_interna_id'])) ? '' : $tipoBusqueda[$value['busqueda_interna_id']];
        //$busquedaExt = ('' || is_null()) ? '' : $tipoBusqueda[$value['busqueda_externa_id']];
        $response[] = array(
          'id' => intval($value['id']),
          'clase' => intval($value['clase_niza_id']) != 0 ? $claseNiza[intval($value['clase_niza_id'])] : '',
          'pais' => intval($value['pais_id']) != 0 ? $paises[intval($value['pais_id'])] : '',
          'responsable' => intval($value['staff_id']) != 0 ? $staff[intval($value['staff_id'])] : '',
          'busqueda_interna' => intval($value['busqueda_interna_id']) != 0 ? $tipoBusqueda[$value['busqueda_interna_id']] : '',
          'busqueda_general' => intval($value['busqueda_externa_id']) != 0 ? $tipoBusqueda[$value['busqueda_externa_id']] : '',
          'acciones' => "<a href='" . admin_url('pi/BusquedasController/edit/' . $value['id']) . "'><i class='fa-solid fa-edit'></i> Editar</a>",
          'marca' => $value['marca']
        );
      }
